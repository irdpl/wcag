...
if (document.querySelector('main')) {
  ...
  if (firstheading) {
    ...
    Array.prototype.forEach.call(toc_elements, function(el, i){ // … .each(…)
      ...
      if (hasclass(el,"ex")) {
        el.insertAdjacentHTML('afterbegin', "<b>Przykład " + excount + ":</b> ");
        excount++;
        if (!hasclass(el, "inap")) {
          apcount = 1;
        }
      ...
      if (hasclass(el,"ap")) {
        el.insertAdjacentHTML('afterbegin', "<b>Podejście " + apcount + ":</b> ");
        apcount++;
      }
  ...
  if ((excolAll !== null) && (excols !== null) && (excolAll.length > 0) && ((excols.length > 1) || (excols2.length > 1))) {
    ...
    Array.prototype.forEach.call(excolAll, function(el, i){
      const expandText = el.dataset.expandText || 'Rozwiń wszystkie sekcje';
      const collapseText = el.dataset.collapseText || 'Zwiń wszystkie sekcje';
  ...
  if (footnoteBox !== null) {
    ...
    Array.prototype.forEach.call(footnoteLinks, function(element, i){
      element.setAttribute('aria-label', t('do przypisu') + ' ' + element.textContent);
      element.setAttribute('title',      t('do przypisu') + ' ' + element.textContent);
    });
    ...
    Array.prototype.forEach.call(footnoteBackLinks, function(element, i){
      element.setAttribute('aria-label', t('wróć do przypisu') + ' ' + element.getAttribute('href').replace('#fnref:','') + ' ' + t('w tekście'));
      element.setAttribute('title',      t('wróć do przypisu') + ' ' + element.getAttribute('href').replace('#fnref:','') + ' ' + t('w tekście'));
    });
