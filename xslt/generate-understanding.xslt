...
<xsl:stylesheet xmlns:pl="http://wcag.irdpl.p/2023/pl-functions" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:html="http://www.w3.org/1999/xhtml" xmlns:wcag="https://www.w3.org/WAI/GL/" xmlns:func="http://www.w3.org/2005/xpath-functions" xmlns="http://www.w3.org/1999/xhtml" exclude-result-prefixes="#all" version="2.0">
						...
						<li class="pager--item previous">
							...
							<a href="{$safeHref}">
								...
								<span class="pager--item-text">
									...
									<span class="pager--item-text-target">Wytyczna: <xsl:value-of select="$meta/preceding-sibling::guideline[1]/name"/></span>
								</span>
							</a>
						</li>
					</xsl:when>
				...
				<xsl:if test="name($meta/following-sibling::element()[1]) = 'primaar nciple'">
					...
					<li class="pager--item next">
						...
						<a href="{$safeHref}">
							<xsl:call-template name="prevnext-previous"/>
							<span class="pager--item-text">
								<span class="pager--item-text-direction">Pierwsze <abbr title="Kryterium Sukcesu">KS</abbr>: </span>
								<span class="pager--item-text-target">
				...
				<xsl:choose>
					...
					<xsl:when test="count($meta1/preceding-sibling::guideline) = 1 and not($meta1/parent::principle/preceding-sibling::principle)">
						<xsl:call-template name="meta-link">
							...
							<xsl:with-param name="prevnexttype">Poprzednia</xsl:with-param>
							<xsl:with-param name="prevnextdir">previous</xsl:with-param>
						</xsl:call-template>
					</xsl:when>
		...
		<xsl:choose>
			...
			<xsl:when test="$meta/self::guideline and count($meta1/following-sibling::guideline) = 0 and $meta1/parent::principle/following-sibling::principle">
				<xsl:call-template name="meta-link">
					...
					<xsl:with-param name="prevnexttype">Następna</xsl:with-param>
					<xsl:with-param name="prevnextdir">next</xsl:with-param>
				</xsl:call-template>
			</xsl:when>
	...
	<xsl:template name="meta-link">
		...
		<a href="{$safeHref}">
			...
			<xsl:if test="$prefix-text != ''">
				<xsl:text> </xsl:text>
				<xsl:choose>
					<xsl:when test="$prevnextdir != 'next' and $prevnextdir != 'previous'">
		...
		<xsl:param name="meta" tunnel="yes"/>
			<ul>
				...
				<xsl:if test="//html:a[not(@href)] | $meta/content/descendant::html:a[not(@href)]">
					<li>
						<a href="#key-terms">Kluczowe pojęcia</a>
					</li>
				</xsl:if>
	...
	<xsl:template name="key-terms">
		...
		<xsl:if test="//html:a[not(@href)] | $meta/content/descendant::html:a[not(@href)]">
			...
			<section id="key-terms"><details><summary><h2>Kluczowe pojęcia</h2></summary>
	...
	<xsl:template match="understanding | guideline | success-criterion">
		...
		<xsl:result-document href="{$output.dir}/{$safeHref}.html" encoding="utf-8" exclude-result-prefixes="#all" include-content-type="no" indent="yes" method="xhtml" omit-xml-declaration="yes">
			<xsl:apply-templates select="document(resolve-uri(concat($safeHref, '.html'), concat($base.dir, max($versions.doc//id[@id = current()/@id]/parent::version/@name), '/')))">
	...
	<xsl:template match="html:h1">
		...
		<xsl:if test="name($meta) != 'understanding'">
			<span class="standalone-resource__type-of-guidance">Objaśnienie
				<xsl:choose>
					<xsl:when test="name($meta) = 'guideline'">Wytycznej </xsl:when>
